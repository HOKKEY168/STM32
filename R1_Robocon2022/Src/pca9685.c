...
PCA9685_STATUS PCA9685_SetBit(uint8_t Register, uint8_t Bit, uint8_t Value) {
	...
	if (HAL_OK
			!= HAL_I2C_Mem_Read(pca9685_i2c, PCA9685_ADDRESS, Register, 1, &tmp,
					1, 10)) {
	...
	if (HAL_OK
			!= HAL_I2C_Mem_Write(pca9685_i2c, PCA9685_ADDRESS, Register, 1,
					&tmp, 1, 10)) {
		return PCA9685_ERROR;
	}
...
PCA9685_STATUS PCA9685_SubaddressRespond(SubaddressBit Subaddress,
		uint8_t Enable) {
...
PCA9685_STATUS PCA9685_SetPwmFrequency(uint16_t Frequency) {
	...
	//
	PCA9685_SleepMode(1);
	HAL_I2C_Mem_Write(pca9685_i2c, PCA9685_ADDRESS, PCA9685_PRESCALE, 1,
			&Prescale, 1, 10); // Write Prescale value
	PCA9685_SleepMode(0);
	PCA9685_RestartMode(1);
	return PCA9685_OK;
}

PCA9685_STATUS PCA9685_SetPwm(uint8_t Channel, uint16_t OnTime,
		uint16_t OffTime) {
	...
	if (HAL_OK
			!= HAL_I2C_Mem_Write(pca9685_i2c, PCA9685_ADDRESS, RegisterAddress,
					1, Message, 4, 10)) {
		return PCA9685_ERROR;
	}
...
PCA9685_STATUS PCA9685_SetServoAngle(uint8_t Channel, float Angle) {
	...
	Value = (Angle - MIN_ANGLE) * ((float) SERVO_MAX - (float) SERVO_MIN)
			/ (MAX_ANGLE - MIN_ANGLE) + (float) SERVO_MIN;
