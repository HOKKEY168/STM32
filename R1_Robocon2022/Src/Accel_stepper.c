 ...
 */
void Accel_Stepper_SetPin(Acceleration_t *Accel_stepper,
		GPIO_TypeDef *step_port, uint16_t step_pin, GPIO_TypeDef *dir_port,
		uint16_t dir_pin) {
 ...
 */
void Accel_Stepper_SetTimer(Acceleration_t *Accel_stepper,
...
//	    	 Generate pulse for stepper driver
		...
		new_step_delay = Accel_stepper->step_delay
				- (((2 * (long) Accel_stepper->step_delay) + rest)
						/ (4 * Accel_stepper->accel_count + 1));
		rest = ((2 * (long) Accel_stepper->step_delay) + rest)
				% (4 * Accel_stepper->accel_count + 1);
...
//	    	 Generate pulse for stepper driver
		...
		new_step_delay = Accel_stepper->step_delay
				+ (((2 * (long) Accel_stepper->step_delay) + rest)
						/ (4 * abs(Accel_stepper->accel_count) + 1));
		rest = ((2 * (long) Accel_stepper->step_delay) + rest)
				% (4 * (long) abs(Accel_stepper->accel_count) + 1);
 ...
 */
void Accel_Stepper_Move(Acceleration_t *Accel_stepper, signed int step,
		unsigned int accel, unsigned int decel, unsigned int rpm)	//acc*100
{
...
//     Use the limit we hit first to calculate decel.
		if (accel_lim <= max_step_lim) {
			...
			Accel_stepper->decel_val =
					-(((long) (max_step_lim * accel)) / decel);
		}
