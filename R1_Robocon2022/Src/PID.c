...
float PID(PIDController *pid, float setpoint, float measurement, float Kp,
		float Ki, float Kd, int i) { // "i" input identity
	/*
	 * -------------------------------------Error signal------------------------------------------
	 */
	...
	pid->integrator[i] = pid->integrator[i]
			+ 0.5f * Ki * pid->T * (error + pid->prevError[i]);
	...
	pid->differentiator[i] = -(2.0f * Kd
			* (measurement - pid->prevMeasurement[i]) /* Note: derivative on measurement, therefore minus sign in front of equation! */
